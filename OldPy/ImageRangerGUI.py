        ...
        for i, p in enumerate(paths):
            try:
                ...
                try:
                    ...
                    # ✅ Optional: convert all thumbnails to RGB to avoid palette/transparency issues
                    if im.mode not in ("RGB", "RGBA"):
                        im = im.convert("RGB")

                except Exception as e:
                    self.gui_log(f"[ERROR] Failed to load {p}: {e}")
                    continue                
...
class LeftSidebar(ttk.Frame):
    def __init__(self, master, on_folder_select, on_sort_change, on_filter_toggle, on_label_filter=None, *args, **kwargs):
        super().__init__(master, *args, **kwargs)
        self.on_folder_select = on_folder_select
        self.on_sort_change = on_sort_change
        self.on_filter_toggle = on_filter_toggle
        self.on_label_filter = on_label_filter  # ✅ callback from main GUI
    ...
    def _build_ui(self):
        ...
        # === Labels panel (metadata filter) ===
        label_frame = ttk.LabelFrame(self, text="Labels")
        label_frame.pack(fill=tk.BOTH, expand=False, padx=6, pady=(3, 6))

        self.lb_labels = tk.Listbox(label_frame, height=6, exportselection=False)
        self.lb_labels.pack(fill=tk.BOTH, expand=True)

        # default content
        self.lb_labels.insert(tk.END, "All Photos")
        self.lb_labels.insert(tk.END, "Unmatched")

        # hook selection to filter callback
        ...
        ttk.Checkbutton(
            control_frame,
            text="Hide small images",
            variable=self.hide_small,
        ...
        ttk.Checkbutton(
            control_frame,
            text="Hide hidden/system",
            variable=self.hide_hidden,
            command=self._trigger_filter
        ).pack(anchor="w", pady=2)
    ...
    def set_label_items(self, names):
        """Replace all labels in the listbox."""
        self.lb_labels.delete(0, tk.END)
        for n in names:
            self.lb_labels.insert(tk.END, n)

    def select_label(self, name):
        """Programmatically select a label by name."""
        for i in range(self.lb_labels.size()):
            if self.lb_labels.get(i) == name:
                self.lb_labels.selection_clear(0, tk.END)
                self.lb_labels.selection_set(i)
                self.lb_labels.see(i)
                break

    def _on_label_select(self, _evt=None):
        """When a label is clicked, inform main GUI."""
        if not self.on_label_filter:
            return
        sel = self.lb_labels.curselection()
        if not sel:
            return
        name = self.lb_labels.get(sel[0])
        print(f"[DEBUG] Label clicked: {name}")  # ✅ Debug log
        self.on_label_filter(name)
        ...
        def task():
            ...
            try:
                try:
                    ...
                    self.gui_log(f"[ERROR] Failed to load {image_path}: {e}")
...
#                    with ImageOps.exif_transpose(Image.open(p)) as im:
#                        im = im.convert("RGB")                                                                      
                    im.thumbnail(thumb_size)
                    bio = im.tobytes()
                    size = im.size
...
#        self.scan_folder(self.selected_folder.get())
    ...
    def _on_label_filter(self, label: str):
...
# --------------------------------------------------    
    ...
    def load_images_recursive(self):
        ...
        self.display_thumbnails()
        self.update_status_bar()
        self._last_displayed_paths = None


    def _compute_filtered_paths(self):
        """Return only image paths matching the current label filter."""
        paths = self.image_paths.copy()

        if not hasattr(self, "photo_meta") or not self.photo_meta:
            return paths

        current_filter = getattr(self, "current_label_filter", "All Photos")
        self.gui_log(f"[DEBUG] Filtering by: {current_filter}")

        filtered = []

        # Pre-normalize photo_meta keys for reliable lookup
        normalized_meta = {
            os.path.normpath(k): v for k, v in self.photo_meta.items()
        }

        for p in paths:
            norm_p = os.path.normpath(p)
            meta = normalized_meta.get(norm_p, {})
            labels = meta.get("labels", set())
            best = meta.get("best_label", None)

            # Debug per image
            self.gui_log(f"[TRACE] {os.path.basename(p)} meta → labels={labels}, best={best}")

            if current_filter in ("All Photos", None):
                filtered.append(p)
            elif current_filter == "Unmatched":
                if not labels or best is None:
                    filtered.append(p)
            else:
                # Match either label set or best label
                if current_filter in labels or current_filter == best:
                    filtered.append(p)

        self.gui_log(f"[DEBUG] Filtered count: {len(filtered)} / {len(paths)}")
        return filtered
...
#        self.update_status_bar()
    ...
    def display_thumbnails(self):
        # Reset thumbnail rebuild state if image list changes drastically
        if len(self.image_paths) != getattr(self, "_last_image_count", -1):
            self._last_display_state = None
        self._last_image_count = len(self.image_paths)
        
        
        # 🧪 No images yet? Bail out.
        if not self.image_paths:
            self.gui_log("[DEBUG] No image paths to display.")
            return

        # 🧪 If canvas isn't sized yet (first layout), retry later
        cw = max(self.canvas.winfo_width(), 1)
        if cw < 10:
            self.root.after(50, self.display_thumbnails)
            return

        # 🧹 Clear previous thumbnail widgets before rebuilding
        for widget in self.scrollable_frame.winfo_children():
            widget.destroy()
        ...
        # 🧭 Sync zoom slider and store size
        zoom_size = self.settings.get("thumbnail_size", (120, 120))[0]
        self.zoom_slider.set(zoom_size)
        self.last_applied_thumb_size = int(zoom_size)

        # 🧮 Compute dynamic columns based on canvas width
        tile_size = self.last_applied_thumb_size + 10  # thumbnail + padding
        columns = max(1, cw // max(tile_size, 1))
        self.dynamic_columns = columns
        self.gui_log(f"[DEBUG] Canvas: {cw}, Columns: {columns}")

        # 🧠 STEP 1: get filtered image list based on current label/filter selection
        paths = self._compute_filtered_paths()
        unique_paths = sorted(set(paths))  # ✅ deduplicate if filter returned duplicates

        if not unique_paths:
            self.gui_log("[DEBUG] No images match current filter. Nothing to display.")
            return

        
        # 🧠 STEP 2: Prevent redundant thumbnail rebuilds (now includes zoom size)
        last_state = getattr(self, "_last_display_state", None)
        current_state = (tuple(unique_paths), self.last_applied_thumb_size)

        if last_state == current_state:
            self.gui_log("[DEBUG] Thumbnail set unchanged and zoom unchanged — skipping rebuild.")
            return

        # ✅ Update state for next time
        self._last_display_state = current_state

        self.gui_log(f"[DEBUG] Displaying {len(unique_paths)} thumbnails after filtering")
        

        # ✅ STEP 3: Build thumbnails for exactly these images
        self._start_thumb_job(unique_paths)
...
#                self.left_sidebar.populate_labels(labels)
                self.left_sidebar.set_label_items(["All Photos", "Unmatched"] + sorted(labels))
