...
class TestPhotoRepository:
    ...
    def test_find_by_id(self, photo_repo: PhotoRepository, init_test_database):
        ...
        with photo_repo.connection() as conn:
            conn.execute("INSERT INTO projects (id, name, folder, mode) VALUES (1, 'test', '/', 'date')")
            conn.execute(
                "INSERT INTO photo_folders (id, name, path, project_id) VALUES (1, 'root', '/', 1)",
            )
            conn.execute(
                "INSERT INTO photo_metadata (path, folder_id, project_id, size_kb, width, height) VALUES (?, ?, ?, ?, ?, ?)",
                ("/test/photo.jpg", 1, 1, 1024.5, 1920, 1080)
            )
    ...
    def test_find_by_path(self, photo_repo: PhotoRepository):
        ...
        with photo_repo.connection() as conn:
            ...
            conn.execute(
                "INSERT INTO photo_metadata (path, folder_id, project_id, size_kb, width, height) VALUES (?, ?, ?, ?, ?, ?)",
                ("/test/unique_photo.jpg", 1, 1, 2048.0, 3840, 2160)
            )

        result = photo_repo.get_by_path("/test/unique_photo.jpg", project_id=1)
        assert result is not None
    ...
    def test_bulk_upsert_insert(self, photo_repo: PhotoRepository):
        ...
        # Row format: (path, folder_id, size_kb, modified, width, height, date_taken, tags,
        #              created_ts, created_date, created_year, gps_lat, gps_lon, content_hash)
        rows = [
            ("/test/photo1.jpg", 1, 1024.0, "2024-10-15 10:00:00", 1920, 1080, "2024:10:15 10:00:00", None, 1729000000, "2024-10-15", 2024, None, None, None),
            ("/test/photo2.jpg", 1, 2048.0, "2024-10-16 11:00:00", 3840, 2160, "2024:10:16 11:00:00", None, 1729100000, "2024-10-16", 2024, None, None, None),
            ("/test/photo3.jpg", 1, 1536.0, "2024-10-17 12:00:00", 2560, 1440, None, "favorite", 1729200000, "2024-10-17", 2024, None, None, None),
        ]

        count = photo_repo.bulk_upsert(rows, project_id=1)
        assert count == 3

        # Verify inserted
        photo1 = photo_repo.get_by_path("/test/photo1.jpg", project_id=1)
        assert photo1 is not None
        assert photo1["width"] == 1920

        photo3 = photo_repo.get_by_path("/test/photo3.jpg", project_id=1)
    ...
    def test_bulk_upsert_update(self, photo_repo: PhotoRepository):
        ...
        with photo_repo.connection() as conn:
            ...
            conn.execute(
                "INSERT INTO photo_metadata (path, folder_id, project_id, size_kb, width, height) VALUES (?, ?, ?, ?, ?, ?)",
                ("/test/update_photo.jpg", 1, 1, 1024.0, 800, 600)
            )

        # Update via bulk upsert
        rows = [
            ("/test/update_photo.jpg", 1, 2048.0, "2024-10-20 10:00:00", 1920, 1080, None, "updated", 1729400000, "2024-10-20", 2024, None, None, None),
        ]
        ...
        # Verify updated
        result = photo_repo.get_by_path("/test/update_photo.jpg", project_id=1)
    ...
    def test_get_all(self, photo_repo: PhotoRepository):
        ...
        with photo_repo.connection() as conn:
            ...
            for i in range(5):
                conn.execute(
                    "INSERT INTO photo_metadata (path, folder_id, project_id, width, height) VALUES (?, ?, ?, ?, ?)",
                    (f"/test/photo{i}.jpg", 1, 1, 800, 600)
                )
    ...
    def test_delete(self, photo_repo: PhotoRepository):
        ...
        with photo_repo.connection() as conn:
            ...
            conn.execute(
                "INSERT INTO photo_metadata (path, folder_id, project_id, width, height) VALUES (?, ?, ?, ?, ?)",
                ("/test/delete_me.jpg", 1, 1, 800, 600)
            )

        # Verify exists
        result = photo_repo.get_by_path("/test/delete_me.jpg", project_id=1)
        assert result is not None
        photo_id = result["id"]
...
class TestFolderRepository:
    ...
    def test_ensure_folder_new(self, folder_repo: FolderRepository):
        ...
        with folder_repo.connection() as conn:
            conn.execute("INSERT INTO projects (id, name, folder, mode) VALUES (1, 'test', '/', 'date')")
        folder_id = folder_repo.ensure_folder("/test/new_folder", "new_folder", parent_id=None, project_id=1)

        assert folder_id is not None
        assert folder_id > 0

        # Verify inserted
        result = folder_repo.get_by_path("/test/new_folder", project_id=1)
        assert result is not None
    ...
    def test_ensure_folder_existing(self, folder_repo: FolderRepository):
        ...
        with folder_repo.connection() as conn:
            conn.execute("INSERT INTO projects (id, name, folder, mode) VALUES (1, 'test', '/', 'date')")
        folder_id1 = folder_repo.ensure_folder("/test/existing", "existing", parent_id=None, project_id=1)

        # Try to ensure again - should return same ID
        folder_id2 = folder_repo.ensure_folder("/test/existing", "existing", parent_id=None, project_id=1)
    ...
    def test_find_by_path(self, folder_repo: FolderRepository):
        ...
        with folder_repo.connection() as conn:
            conn.execute("INSERT INTO projects (id, name, folder, mode) VALUES (1, 'test', '/', 'date')")
        folder_repo.ensure_folder("/test/find_me", "find_me", parent_id=None, project_id=1)

        result = folder_repo.get_by_path("/test/find_me", project_id=1)
        assert result is not None
    ...
    def test_get_children(self, folder_repo: FolderRepository):
        ...
        with folder_repo.connection() as conn:
            conn.execute("INSERT INTO projects (id, name, folder, mode) VALUES (1, 'test', '/', 'date')")
        parent_id = folder_repo.ensure_folder("/test/parent", "parent", parent_id=None, project_id=1)

        # Create children
        folder_repo.ensure_folder("/test/parent/child1", "child1", parent_id=parent_id, project_id=1)
        folder_repo.ensure_folder("/test/parent/child2", "child2", parent_id=parent_id, project_id=1)
        folder_repo.ensure_folder("/test/parent/child3", "child3", parent_id=parent_id, project_id=1)

        children = folder_repo.get_children(parent_id, project_id=1)
    ...
    def test_update_photo_count(self, folder_repo: FolderRepository):
        ...
        with folder_repo.connection() as conn:
            conn.execute("INSERT INTO projects (id, name, folder, mode) VALUES (1, 'test', '/', 'date')")
        folder_id = folder_repo.ensure_folder("/test/photos", "photos", parent_id=None, project_id=1)
    ...
    def test_hierarchy_integrity(self, folder_repo: FolderRepository):
        ...
        with folder_repo.connection() as conn:
            conn.execute("INSERT INTO projects (id, name, folder, mode) VALUES (1, 'test', '/', 'date')")
        root_id = folder_repo.ensure_folder("/test/root", "root", parent_id=None, project_id=1)
        level1_id = folder_repo.ensure_folder("/test/root/level1", "level1", parent_id=root_id, project_id=1)
        level2_id = folder_repo.ensure_folder("/test/root/level1/level2", "level2", parent_id=level1_id, project_id=1)
...
class TestProjectRepository:
    ...
    def test_get_branches(self, project_repo: ProjectRepository):
        ...
        assert all("branch_key" in b for b in branches)
