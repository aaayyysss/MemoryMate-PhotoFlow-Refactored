...
class PersonSearchService:
    ...
    def compute_person_facet_identity_aware(
        self, paths: List[str], max_people: int = 10
    ) -> List[Dict[str, Any]]:
        """
        UX-11: Compute person facets through the durable identity layer.

        Resolves branch_keys to person_identity records, coalescing
        multiple clusters that belong to the same identity.
        Hidden identities are excluded. Protected identities sort first.

        Returns list of dicts:
            [{"identity_id": "pid_...", "display_name": "John", "count": 15, "is_protected": True}, ...]
        """
        if not paths:
            return []
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                try:
                    conn.execute("SELECT 1 FROM person_identity LIMIT 1")
                except Exception:
                    return []  # Identity tables not yet created, caller should fall back

                placeholders = ",".join("?" * len(paths))
                # Get branch_key → image_path hits from face_crops
                cursor = conn.execute(
                    f"SELECT branch_key, image_path FROM face_crops "
                    f"WHERE project_id = ? AND image_path IN ({placeholders})",
                    [self.project_id] + list(paths)
                ...
                # Group by branch_key
                branch_paths: Dict[str, set] = {}
                for row in cursor.fetchall():
                    bk = row["branch_key"]
                    if bk:
                        branch_paths.setdefault(bk, set()).add(row["image_path"])

                if not branch_paths:
                    return []

                # Resolve branch_keys to identities via identity_cluster_link
                identity_hits: Dict[str, Dict[str, Any]] = {}
                for bk, bk_paths in branch_paths.items():
                    id_cursor = conn.execute(
                        """
                        SELECT pi.identity_id, pi.display_name,
                               pi.canonical_cluster_id, pi.is_protected, pi.is_hidden
                        FROM person_identity pi
                        INNER JOIN identity_cluster_link icl
                            ON icl.identity_id = pi.identity_id AND icl.is_active = 1
                    ...
                    if id_row and not id_row[4]:  # not hidden
                        iid = id_row[0]
                        bucket = identity_hits.setdefault(iid, {
                            "identity_id": iid,
                            "display_name": id_row[1] or id_row[2] or iid,
                            "canonical_cluster_id": id_row[2],
                            "is_protected": bool(id_row[3]),
                            "photo_paths": set(),
                        })
                        bucket["photo_paths"].update(bk_paths)
                    elif not id_row:
                        # No identity — use branch_key as-is (legacy fallback)
                        self._load_name_cache()
                        name = (self._branch_to_name or {}).get(bk, bk)
                        bucket = identity_hits.setdefault(f"_bk_{bk}", {
                            "identity_id": bk,
                            "display_name": name,
                ...
                facets = []
                for bucket in identity_hits.values():
                    facets.append({
                        "identity_id": bucket["identity_id"],
                        "display_name": bucket["display_name"],
                        "count": len(bucket["photo_paths"]),
                        "is_protected": bucket["is_protected"],
                    })

                facets.sort(key=lambda x: (
                    not x["is_protected"],
                    -x["count"],
                    x["display_name"].lower(),
                ))
                return facets[:max_people]

        except Exception as e:
            logger.debug(f"[PersonSearch] compute_person_facet_identity_aware failed: {e}")
            return []
